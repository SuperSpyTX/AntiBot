...
public class CommandHandler
{
    ...
    public boolean handle(CommandSender sender, Command cmd, String commandLabel, String[] args)
    {
        ...
        if (args[0].compareToIgnoreCase("run") == 0)
        {
            ...
            {
                if (!antibot.getDataTrack().getLoginTracker().reanibo && Settings.whiteList)
                {
                    if (Settings.notify)
                    {
                    ...
                    antibot.getDataTrack().getLoginTracker().reanibo = true;
                    antibot.getUtility().getDebug().debug("Tripswitched!");
                    antibot.getDataTrack().getLoginTracker().kickConnected();
                    antibot.getDataTrack().getLoginTracker().botattempt = System.currentTimeMillis();
                    antibot.getDataTrack().getLoginTracker().botcts += 1;
                }
        ...
        if (args[0].compareToIgnoreCase("info") == 0)
        {
            if (Permissions.ADMIN_INFO.getPermission(player, sender))
            {
                ...
                long math = antibot.getDataTrack().getLoginTracker().time - antibot.getDataTrack().getLoginTracker().lasttime;
                antibot.debug("Secs between last login: " + math, sender);
                antibot.debug("Current Intervals: " + Settings.interval, sender);
                antibot.debug("Logged in: " + antibot.getDataTrack().getLoginTracker().botcts, sender);
